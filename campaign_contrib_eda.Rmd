...
```
```{r}
ggplot(data = cand.date.count %>% inner_join(top_5_cand), aes(x = contb_receipt_dt, y = n)) +
  geom_bar(stat = "identity") +
  facet_wrap(~cand_nm) +
  horiz_lab
```
...
```{r}
ggplot(data = city.receipts.cand %>% filter(!cand_nm %in% c("Clinton, Hillary Rodham")), 
...
```{r}
ggplot(data = city.receipts.cand %>% filter(cand_party %in% c("REPUBLICAN", "DEMOCRATIC")),
...
```{r}
ggplot(data = date.receipts.cand, aes(x = contb_receipt_dt, y = contb_receipt_amt, fill = cand_nm)) +
  geom_bar(stat="identity") +
  horiz_lab
```
...
```{r}
ggplot(data = date.receipts.cand.t5, aes(x = contb_receipt_dt, y = contb_receipt_amt)) +
  geom_bar(stat="identity") +
  facet_wrap(~ cand_nm) +
  horiz_lab
```
...
```{r}
ggplot(data = date.receipts.cand, aes(x = contb_receipt_dt, y = contb_receipt_amt, fill = cand_party)) +
  geom_bar(stat="identity") +
  horiz_lab
```
...
#### Democrats
```{r}
ggplot(data = filter(date.receipts.cand, cand_party == "DEMOCRATIC"), aes(x = contb_receipt_dt, y = contb_receipt_amt, fill = cand_nm)) +
  geom_bar(stat="identity") +
  horiz_lab
```
...
#### Republicans
```{r}
ggplot(data = filter(date.receipts.cand, cand_party == "REPUBLICAN"), aes(x = contb_receipt_dt, y = contb_receipt_amt, fill = cand_nm)) +
  geom_bar(stat="identity") +
  horiz_lab
```
...
ggplot(data = date.receipts.summary, aes(x = contb_receipt_dt)) +
  ...
  geom_bar(aes(y = median_receipt, fill = "Median"), stat = "identity") +
  horiz_lab +
  labs(title = "Mean/Median Receipts per Month", y = "Receipt Amount", x = "Date", fill = "Stat")
```

## Description Two

This plot of the mean and median receipts per month shows a difference between the contributions that a candidate gets throughout the election process. Both the mean and the median is relatively high earlier in the election. As the candidates get further into the election, the median decreases and the mean follows it slowly until after the election when the mean dives to negative.
...
ggplot(data = filter(date.receipts.cand, cand_party == "DEMOCRATIC"), aes(x = contb_receipt_dt, y = contb_receipt_amt, fill = cand_nm)) +
  geom_bar(stat="identity") +
  horiz_lab +
  labs(title = "Receipts per Month by Democratic Candidate", x = "Date", y = "Dollars")
```
...
ggplot(data = filter(date.receipts.cand, cand_party == "REPUBLICAN"), aes(x = contb_receipt_dt, y = contb_receipt_amt, fill = cand_nm)) +
  geom_bar(stat="identity") +
  horiz_lab +
  labs(title = "Receipts per Month by Republican Candidate", x = "Date", y = "Dollars")
```
...
Figuring out how to work with so many categorical features was an interesting challenge. Using summary statistics to explore the relationships between features seems to have provided useful results.
